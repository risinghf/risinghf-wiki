...
[参考 中继使用说明书](../../01.IoT无线通讯组网/03.中继/01.RHF3MR01/04.使用说明书.md)
...
⽤⼾创建应⽤, 推送接口可以填写 `http` 或 `https` 两种方式, 用户也可在 `Header` 字段中填写 `Token` 信息, 增加用户推送接口的安全性;
...
![创建应用](https://risinghf-wiki.oss-cn-shenzhen.aliyuncs.com/upload/img/20230619170518.png)

### 3.2 创建设备

⽤⼾创建设备,在添加设备弹窗选择产品类型为"雀桥中继", 按要求填写 `DevEUI` `AppEUI` `AppKey` 频率区域以及设备类型, 来注册设备

![创建设备](https://risinghf-wiki.oss-cn-shenzhen.aliyuncs.com/upload/img/20230619170539.png)
