...
```
完整TOPIC示例：user/1/device/474abd91003xxxx/downlink
{{device_eui}} 为设备ID，{{uid}} 为用户ID，具体可在IoTSqaure平台中的用户管理中查看开发者对应的用户ID。
```
...
| **字段名**    | **类型** | **描述**     |
|------------|--------|------------|
| **token**  | string | 用户可以在 `IoTSqaure` 后台的用户详情中找到 `API Token` 字段，例如：**`tl1lzqMMueQUVI+Eyotbnw==`** |
| **params** | params | 参数         |
...
### 4.3 数据格式
#### 4.3.1 Protobuf格式
4.2.4版本以前的默认格式，开发者 `MQTT` 客户端在经过 `proto buffer` 反序列化步骤之后将得到完整的 `JSON`。
