...
| productType     | string | 产品类型：rhf3mr01(中继),rhf3m485(透传设备),lorawan(标准设备)；3.4.15新增 |
| deviceProfileId | uint   | 设备配置ID，可选，标准中继配置；4.3.1版本新增                              |::: warning
...
| devName         | string   | 是    | 节点名称                                                      |
| deviceProfileId | uint     | 否    | 设备配置ID，可选，标准中继配置；4.3.1版本新增                                |
| productType     | string   | 是    | 产品类型：rhf3mr01(中继),rhf3m485(透传设备),lorawan(标准设备)；3.4.15版本新增 |
| supportClassB   | bool  	  | 是    | 是否支持ClassB，变更后需OTAA重新入网或ABP重置，4.2版本新增                            |
| supportClassC   | bool   	 | 是    | 是否支持ClassC，变更后需OTAA重新入网或ABP重置，4.2版本新增                             |
| supportClassR   | bool   	 | 是    | 是否支持ClassR，变更后需OTAA重新入网或ABP重置，4.2版本新增                             |
...
| 字段名    | 类型     | 必填 | 描述                                           |
|:-------|:-------|:---|:---------------------------------------------|
| devEUI | string | 否  | `16` 位 `16` 进制符串，节点设备唯一标识符。不传时查询所有设备的待下行队列长度 |
...
| gwName       | string            | 网关名称                             |
| gwType       | string            | 网关类型                             |
| region       | string            | 频率区域，如 `cn470`；4.3.2新增           |
| subnet       | string            | 子网，如 `ch_08-15`；4.3.2新增          |
...
| altitude     | float             | 海拔，需要带 `GPS` 的网关;3.4.15版本移除      |
...
|-------------|---------|-------------------------------------------------------|
| nwkGw       | string  | 网关接口名称                                                |
| localIP     | string  | 本地IP（pico复用 lteNetIP）                                 |
| wanIP       | string  | 客户端侧的公网IP                                             |
| nwkDelay    | string  | 网络延迟（ping结果，序列化后为字符串格式，如"15ms"）                       |
| ltess       | string  | 4G信号强度，通过get_lterssi命令获取（pico复用 lteNetRssi）           |
| sensorData  | string  | 传感器数据（原结构体字段名拼写修正为规范命名）                               |
| createtime  | string  | 数据上报时间（序列化后为RFC3339格式，如"2024-05-20T13:14:15+08:00"）   |
| ethState    | boolean | 以太网连接状态：true=已连接，false=断开                             |
| lteState    | boolean | LTE拨号状态：true=拨号成功，false=未拨号                           |
| apn         | string  | APN接入点（4G连接时为CMNET/CTNET，以太网连接时为空字符串）                 |
| mode        | string  | LTE拨号的网络制式（如TDD-LTE/FDD-LTE）                          |
| rssi        | number  | LTE当前网络信号质量（单位：dBm，合法范围：-113~-51）                     |
| rsrp        | number  | LTE参考信号接收功率（单位：dBm，合法范围：-140~-44）                     |
| rsrq        | number  | LTE参考信号接收质量（单位：dB，合法范围：-19.5~-3），修正原JSON标签拼写错误（原rspq） |
| snr         | number  | LTE信噪比（单位：dB，合法范围：-20~30）                             |
| imsi        | string  | SIM卡IMSI（国际移动用户识别码，由MCC/MNC/MSIN组成）                   |
| cellid      | string  | LTE拨号的小区ID                                            |
| lacid       | string  | LTE拨号的位置区ID                                           |
| imei        | string  | LTE模组IMEI（设备识别码，修正原注释误写）                              |
| iccid       | string  | SIM卡ICCID（集成电路卡识别码）                                   |
| dialStatus  | integer | SIM卡及拨号状态：0=未插卡，1=未识别SIM卡，2=未拨号，3=拨号成功                |
| lteManuf    | string  | LTE模组厂家（如华为、移远）                                       |
| lteModel    | string  | LTE模组型号（如EC20、AG35）                                   |
| lteRevision | string  | LTE模组固件版本                                             |
| lteEsn      | string  | LTE模组ESN（电子序列号）                                       |
| lteGcap     | string  | LTE模组能力集（如支持频段）                                       |
| meid        | string  | LTE模组MEID（移动设备识别码）                                    |
| rxlev       | string  | LTE拨号的接收电平（单位：dBm）                                    |
| lteSim      | string  | 拨号使用的SIM类型。esim为内置SIM卡，usim为外置SIM卡（4.3.2版本新增）         |
...
| 字段名字                  | 类型                  | 描述                                    |
|:----------------------|:--------------------|:--------------------------------------|
| uptime                | int                 | 网关启动后的运行时长，秒                          |
| sdkUptime             | int                 | 网关bridge启动后的运行时长                      |
| cpuUsage              | string              | cpu使用率                                |
| memUsage              | string              | 内存使用率                                 |
| rootDiskUsage         | string              | 根分区使用率                                |
| logDiskUsage          | string              | 日志分区使用率                               |
| pktfwdRunning         | bool                | pktfwd是否在运行中                          |
| createtime            | uint64              | 最近一条信息上报的毫秒时间戳                        |
| temperature           | string              | 温度                                    |
| powerState            | bool                | true为已接外部电源，false为未接外部电源              |
| powerVoltage          | float64             | 外部电源电压，单位：伏特（4.3.2版本新增）               |
| batteryState          | bool                | true为已接备电锂电池，false为未接备电锂电池            |
| batteryLevel          | uint                | 备电锂电池电量百分比，范围：0～100                   |
| batteryVoltage        | float64             | 备电锂电池电压，单位：伏特                         |
| sbatteryState         | bool                | true为已接太阳能电池，false为未接太阳能电池（4.3.2版本新增） |
| sbatteryLevel         | uint                | 太阳能电池电量百分比，范围：0～100                   |
| sbatteryVoltage       | float64             | 太阳能电池电压，单位：伏特                         |
| sbatteryCollectObject | SbatteryCollectData | 太阳能电池采集数据（4.3.2版本新增）                  |

**SbatteryCollectData结构**(4.3.2版本新增):

| 字段名字         | 类型                   | 描述     |
|:-------------|:---------------------|:-------|
| batteryBasic | SbatteryBasic        | 电池基本信息 |
| batteryCell  | SbatteryCell         | 电池单体信息 |
| generalParam | SbatteryGeneralParam | 通用参数   |
| miscParam    | SbatteryMiscParam    | Misc参数 |

**SbatteryBasic结构**:

| 字段名字                          | 类型                  | 描述            |
|:------------------------------|:--------------------|:--------------|
| controlInfo                   | SbatteryControlInfo | 控制信息          |
| warnInfo                      | SbatteryWarnInfo    | 告警信息          |
| remainingAh                   | float64             | 剩余AH数         |
| soc                           | float64             | 剩余电量SoC       |
| sohPwm                        | float64             | PWM占空比        |
| power                         | float64             | 电池功率          |
| totalVoltage                  | float64             | 电池总电压         |
| totalCurrent                  | float64             | 电池总电流         |
| totalCurrentDirection         | string              | 电池总电流方向       |
| pvChargeCurrent               | float64             | 电池充电（光伏端）电流   |
| pvChargeCurrentDirection      | string              | 电池充电（光伏端）电流方向 |
| loadDischargeCurrent          | float64             | 电池放电（负载端）电流   |
| loadDischargeCurrentDirection | string              | 电池放电（负载端）电流方向 |
| temp1                         | float64             | 温度1           |
| temp2                         | float64             | 温度2           |
| temp3                         | float64             | 温度3           |
| clampingVoltage               | float64             | 钳位电压          |
| soh                           | float64             | SOH           |
| socPwm                        | float64             | PWM占空比        |
| pvVoltage                     | float64             | 光伏电压          |
| pvCurrent                     | float64             | 光伏电流          |
| pvCurrentDirection            | string              | 光伏电流方向        |
| gridCurrent                   | float64             | 市电电流          |
| gridCurrentDirection          | string              | 市电电流方向        |
| tempProtect                   | float64             | 温度保护          |
| dischargeProtect              | float64             | 放电保护          |
| chargeProtect                 | float64             | 充电保护          |

**SbatteryControlInfo结构**:

| 字段名字            | 类型     | 描述   |
|:----------------|:-------|:-----|
| heating         | string | 加热   |
| heatDissipation | string | 散热   |
| buzzing         | string | 蜂鸣   |
| overTempProt    | string | 过温保护 |
| dischargeSwitch | string | 放电开关 |
| chargingSwitch  | string | 充电开关 |
| chargingOn      | string | 充电开启 |
| chargingConn    | string | 充电连接 |

**SbatteryWarnInfo结构**:

| 字段名字                      | 类型      | 描述     |
|:--------------------------|:--------|:-------|
| highTempProt              | string  | 高温保护   |
| severeOvercurrent         | string  | 严重过流   |
| chargingOvercurrent       | string  | 充电过流   |
| batteryOvercharge         | string  | 电池过充   |
| batteryDepleted           | string  | 电量耗尽   |
| highTempWarn              | string  | 高温告警   |
| dischargeOvercurrent      | string  | 放电过流   |
| lowBattery                | string  | 电量过低   |
| excessivePressure         | string  | 压差过高   |
| lowTempCharging           | string  | 充电低温   |
| lowTempDischarge          | string  | 放电低温   |
| chargingExcessivePressure | string  | 充电压差过高 |
| shortCircuitFault         | string  | 存在短路故障 |
| highHumidity              | string  | 湿度过高   |
| insulationTestAlarm       | string  | 绝缘检测告警 |
| incompatibleCharger       | string  | 充电器不匹配 |
| faultCodes                | float64 | 故障码    |
| faultLevel                | float64 | 故障等级   |

**SbatteryCell结构**:

| 字段名字         | 类型      | 描述      |
|:-------------|:--------|:--------|
| cellCount    | float64 | 电芯个数    |
| cell1Voltage | float64 | 电芯1单体电压 |
| cell2Voltage | float64 | 电芯2单体电压 |
| cell3Voltage | float64 | 电芯3单体电压 |

**SbatteryGeneralParam结构**:

| 字段名字                  | 类型                 | 描述       |
|:----------------------|:-------------------|:---------|
| lvl1DischargeOc       | float64            | 一级放电过流   |
| lvl1DischargeOcDelay  | float64            | 一级放电过流延时 |
| lvl2DischargeOc       | float64            | 二级放电过流   |
| lvl2DischargeOcDelay  | float64            | 二级放电过流延时 |
| cellUv                | float64            | 单体过放电压   |
| cellUvRecover         | float64            | 单体过放恢复电压 |
| cellOvRecover         | float64            | 单体过充恢复电压 |
| cellOv                | float64            | 单体过充电压   |
| batOtAlarm            | float64            | 电池高温告警   |
| batOtProtect          | float64            | 电池高温保护   |
| sysOtAlarm            | float64            | 系统高温告警   |
| sysOtProtect          | float64            | 系统高温保护   |
| cellCount             | float64            | 电池节数     |
| ratedCapacity         | float64            | 额定电量值    |
| currentRange          | float64            | 电流量程     |
| currentOffset         | float64            | 电流零点偏移   |
| voltageCalib          | float64            | 电压校准系数   |
| chargeCurrentOffset   | float64            | 充电电流零点偏移 |
| currentNoiseThreshold | float64            | 电流误检门限   |
| chargeOc              | float64            | 充电过流门限   |
| fullVoltage           | float64            | 电池满电电压   |
| floatVoltage          | float64            | 单体浮充电压   |
| balanceStartVoltage   | float64            | 均衡开启电压   |
| chargeLtProtect       | float64            | 充电低温保护   |
| dischargeLtProtect    | float64            | 放电低温保护   |
| heatStartTemp         | float64            | 加热触发温度   |
| heatStopTemp          | float64            | 加热关闭温度   |
| chargeLtRecover       | float64            | 充电低温恢复温度 |
| dischargeLtRecover    | float64            | 放电低温恢复温度 |
| tempSensorEnable      | float64            | 温度探头使能   |
| funcSwitch            | SbatteryFuncSwitch | 程序功能开关   |
| diffVoltageSection    | float64            | 压差告警分段电压 |
| diffVoltageHigh       | float64            | 高段压差告警   |
| diffVoltageLow        | float64            | 低段压差告警   |
| gridSwitchVoltage     | float64            | 市电切换电压   |
| chargeDiffHigh        | float64            | 充电高段压差告警 |
| devId                 | string             | 设备ID     |
| totalUv               | float64            | 总压欠压保护   |
| totalOv               | float64            | 总压过压保护   |
| sensorEnable          | float64            | 传感器使能    |
| dischargeOt           | float64            | 放电高温保护阈值 |
| socRecover            | float64            | 过放恢复SOC  |
| gridCurrentOffset     | float64            | 市电电流偏移   |
| chargeDiffLow         | float64            | 充电低段压差告警 |
| gridCurrentRange      | float64            | 市电电流量程   |
| loadCurrentRange      | float64            | 负载电流量程   |

**SbatteryFuncSwitch结构**:

| 字段名字               | 类型     | 描述     |
|:-------------------|:-------|:-------|
| batteryPriority    | string | 电池优先   |
| reportRealtimeInfo | string | 上报实时信息 |
| batteryCells       | string | 电芯     |
| shortCircuitProt   | string | 短路保护   |

**SbatteryMiscParam结构**:

| 字段名字        | 类型      | 描述      |
|:------------|:--------|:--------|
| dischargeAh | float64 | 累计放电AH数 |
| calibAh     | float64 | 校准AH数   |
| remainAh    | float64 | 剩余AH数   |
| chargeAh    | float64 | 累计充电AH数 |
| pvDaily     | float64 | 光伏日发电量  |
| pvTotal     | float64 | 光伏累计发电量 |
| loadDaily   | float64 | 负载日用电量  |
| loadTotal   | float64 | 负载累计用电量 |
| forceCtrl   | float64 | 充放电强制开关 |
