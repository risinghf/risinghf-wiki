...
3. ApiToken: 调用IoTSquare的openAPI时，需要有API Token进行鉴权。用户可以在IoTSqaure界面的用户详情中找到API Token字段，例如：`tl1lzqMMueQUVI+Eyotbnw==`，那么用户在调用接口时，需要设置HTTP请求头`X-Access-Token`字段值为`tl1lzqMMueQUVI+Eyotbnw==`。
