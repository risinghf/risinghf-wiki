...
RHF2S208D是一款由瑞兴恒方网络（深圳）有限公司自主设计和研发的主打室外应用场景的高性能的物联网LoRaWAN网关， 其内部搭载了raspberry pi CM4作为主控，CPU基于 64 位 ARM Cortex-A72 架构，4核，主频为 1.5GHz，具有强大的数据处理能力。此外，RHF2S208D还配备了2个SX1302 LoRa模组、1个GPS模块、1个以太网接口、1个LTE模组、1个WiFi模组、1个温度传感器和1个太阳能板RS485串口采集接口。支持DC、POE、锂电池、太能阳电池等多种供电方式，满足各种不同的安装场景。

本文将详细地介绍如何使用RHF2S208D内置的Web管理平台来操作网关，然后配合RisingHF提供的LoRaWAN服务——IoTSquare和一个节点，通过搭建一个完整的通信示例来演示网关的实际使用过程，接着介绍升级固件的方法，最后本文会给出一些常见问题的解决建议，希望能帮助到用户。
...
*   支持解析太阳能电池RS485的数据，并上传到云服务器。
*   内置易用的Web网关管理平台。
*   支持LoRaWAN 470 全半双模式（选配）
*   支持LoRaWAN 16信道（选配）

### 网关外部物理接口

网关总共有 4 个天线端口，1 个外部电源接入端口，1 个 RJ45 端口，1个RS485串口，1个调试串口。
...
RHF2S208D 使用标配的 AC/DC 工业电源进行供电。RHF2S208D 出厂均配备一个高可靠性工业级 IP67 防水 AC/DC 电源。另外，还能使用机房里的POE供电。在户外也能接太阳能板和电池供电。针对RHF2S208DBxx 系列网关，还内置了一个 3200mAh 的磷酸铁锂可充电电池作为备用电源。所有电源均可接入设备，设备自身能够智能地按照 AC/DC 工业电源 > POE电源 > 太阳能电池 > 备用电池的优先级顺序选择工作电源。

### 回传网络

RHF2S208D 支持两种回传网络方式，如以太网和LTE。

以太网：使用网线将网关的 RJ45 口连接到上一级路由器的 LAN口则可以实现 10/100m 以太网连接到服务器。

LTE：请打开设备上盖，并插入 SIM 卡后重新上电启动系统即可。如果设备焊接了esim卡，那么上电将自动拨号。

你可以登录内置 web 界面来配置回传网络的优先级。

### 调试接口

RHF2S208D 提供了多种使用调试接口。
...
RHF2S208D 内置的Web管理平台是专为用户提供操作网关的入口，它非常的便捷易用，用户可以通过它来轻松地管理、操作自己的网关。本章将详细介绍管理平台的各个部分及其使用方法。
...
在电脑上扫描 WiFi 信号，找到 SSID 为 RHF2S208D_XXXXXX 的设备并连接。电脑连接上设备后，默认IP 192.168.8.1。得到设备IP地址之后，通过浏览器打开192.168.8.1进入设备的内置web界面。WiFi 默认的 SSID 和密码:
...
![image-20260618135519269](https://risinghf-wiki.oss-cn-shenzhen.aliyuncs.com/upload/img/image-20260618135519269.png)
...
![image-20260618135556828](https://risinghf-wiki.oss-cn-shenzhen.aliyuncs.com/upload/img/image-20260618135556828.png)
...
*   外部电源电压：当接入外部电源时，显示接入电压。
*   电池供电：对于RHF2S208DExx系列设备，显示不支持。对于RHF2S208DBxx系列设备，当接锂电池时，显示电池电量百分比；当未接锂电池，提示“异常”。
*   电池电压：太阳能电池的电压。
...
底噪扫描功能操作非常简便，只需要填入起始频点和终止频点，然后点击“开始扫描”即可。如果你只输入起始频点，则默认的扫描带宽2MHz，默认的步进为200kHz。扫描的带宽越宽，需要花费的时间就越长。一般建议只扫描2MHz或者5MHz以覆盖目标工作频段即可。**注意：硬件的原因导致RHF2S208Dxxx-470系列只能扫描470~490Mhz，RHF2S208Dxxx-868系列只能扫描862~876Mhz，RHF2S208Dxxx-915系列只能扫描902~928Mhz。**
...
网关ID：填入网关 ID（注意不同的服务器会使用不同的网关ID命名规则）。RHF2S208D默认使用以太网的MAC地址中间插入FFFE作为网关ID，用户可以根据自己的需求修改。
...
![image-20260618145800836](https://risinghf-wiki.oss-cn-shenzhen.aliyuncs.com/upload/img/image-20260618145800836.png)
...
![image-20260618150829916](https://risinghf-wiki.oss-cn-shenzhen.aliyuncs.com/upload/img/image-20260618150829916.png)
...
设备一共提供两种网络接口br-wan（有线网络）和ppp0（LTE拨号），该流量统计仅供参考。

![image-20260618151040673](https://risinghf-wiki.oss-cn-shenzhen.aliyuncs.com/upload/img/image-20260618151040673.png)
...
如果接入了太阳能电池并接上了RS485串口，将会从太阳能板读出设备此时的工作状态。包含了电池基本信息、电池单体信息、通用参数和Misc参数。（需要采购我司指定型号的太阳能电池才能显示太阳能电池数据）

![image-20260618151117299](https://risinghf-wiki.oss-cn-shenzhen.aliyuncs.com/upload/img/image-20260618151117299.png)
...
当以太网（eth0）和LTE（ppp0）网络状态发生改变时，网络日志会更新。
...
![image-20260721162919543](https://risinghf-wiki.oss-cn-shenzhen.aliyuncs.com/upload/img/image-20260721162919543.png)
...
通过配置回传网络优先级，可以优先选择 10/100m 以太网和LTE 4G 优先网络方式作为回传网络。默认网关将以太网作为优先网络，当用户同时使用以太网和 4G 时，以太网网线被拔时，自动切换成 4G。当用户将网关回传网络优先级修改成 4G 优先时，当 4G 掉线，网关会自动切换成有线网络。

![image-20260618153133149](https://risinghf-wiki.oss-cn-shenzhen.aliyuncs.com/upload/img/image-20260618153133149.png)
...
此外，还需要一台电脑和一个连接互联网的路由器，并将RHF2S208D网关和电脑连接到路由器组成局域网（网关使用以太网连接），然后在电脑上登录路由器管理界面查询网关的IP地址，以便后续登录Web管理平台操作网关。
...
![image-20260618154337570](https://risinghf-wiki.oss-cn-shenzhen.aliyuncs.com/upload/img/image-20260618154337570.png)
...
设备具有8个指示功能的LED灯提示设备正在处于何种工作状态下，但是没有接到外壳，用户需要打开盖子才能看到指示。
...
<table>
    ...
    <tr>
        <td>灭</td>
        <td>太阳能电池没有充电，或者充满停止充电</td>
    </tr>
    <tr>
        <td>LED203</td>
        <td>长亮</td>
        <td>输入电源灯（包含外部电源、电池等）</td>
    </tr>
    <tr>
        <td>LED400</td>
        <td>闪烁</td>
    ...
    <tr>
        ...
        <td>慢闪（1800ms灭/200ms亮）</td>
        <td>4G模组待机</td>
    </tr>
    <tr>
        <td>慢闪（200ms灭/1800ms亮）</td>
        <td>搜网状态</td>
    </tr>
    <tr>
        <td>快闪（125ms灭/125ms亮）</td>
        <td>拨号成功，数据传输状态</td>
    </tr>
</table>
...
RHF2S208D仅支持USB的方式升级固件。

升级前需要满足3个前提：

1. 下载并安装rpiboot：https://github.com/raspberrypi/usbboot/releases/download/windows-v1.1/rpiboot_setup.exe
2. 下载并安装Raspberry Pi Imager：https://downloads.raspberrypi.com/imager/imager_latest.exe
3. 咨询我司获取最新版本的RHF2S208D固件。

升级步骤如下：

1. 断开RHF2S208D电源。
2. 使用type C USB线将RHF2S208D（U401）连到至PC。
3. 重新将RHF2S208D上电，进入烧录模式。
4. 进入rpiboot的安装目录，打开rpiboot.exe。

![捕获](https://risinghf-wiki.oss-cn-shenzhen.aliyuncs.com/upload/img/%E6%8D%95%E8%8E%B7.PNG)

5. 桌面将会出现命令终端，然后消失，多了一个bootfs。

![捕获1](https://risinghf-wiki.oss-cn-shenzhen.aliyuncs.com/upload/img/%E6%8D%95%E8%8E%B71.PNG)

6. 打开Raspberry Pi Imager。
...
Q8：为什么菜单栏“太阳能电池采集数据”没有数据？

A8：太阳能电池采集的数据只针对一款太阳能电池做适配，因此需要购买我司指定的太阳能电池。如果没有接电池（包含接RS485）或者不是指定太阳能电池，将无法采集到有效的数据。