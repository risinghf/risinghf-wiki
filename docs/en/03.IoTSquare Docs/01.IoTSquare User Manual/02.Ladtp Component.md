...
The user creates a device, the product type selects "high isolation RS485 data transparent transmission module", and fills in `DevEUI` `AppEUI` `AppKey` `frequency region` `device type` as required to register the device
