...
2. Request Method：MQTT

3. Topic: `user/{{uid}}/device/{{device_eui}}/downlink`

```
Complete TOPIC example：user/1/device/474abd91003xxxx/downlink
...
2. Topic: `user/{{uid}}/device/{{device_eui}}/downlink`
...
4. Complete example of publishing topic:
...
### 4.3 Data Format
#### 4.3.1 Protobuf Format
Prior to version 4.2.4, Protobuf used the default format.
...
#### 4.3.2 JSON Format
Available in version 4.2.4 and later.
...
```json
{
  ...
  "applicationID": "1394",
  "msgType": "uplink",
  "timestamp": "1768546093830",
  "gatewayID": "b827ebfffe9aae48",
  "rssi": -37,
  "snr": 10,
  "frequency": 482.7,
  "adr": true,
...
## 6.Receive gateway status data using the HTTP protocol

When creating a gateway, you can configure the push address, such as `http://test.xyz/api/gateway`,
Then the gateway heartbeat will be pushed to the API `http://test.xyz/api/gateway/gwstat`,
Any changes to the gateway's online status will be pushed to the API at `http://test.xyz/api/gateway/gwconnection`

### 6.1 Gateway online status push

Below is an example of gateway online status push:
...
| online      | bool  | Online: true (online); false (offline)                       |
| mqtt        | int   | true: mqtt ; false：pktfwd                                   |
| latitude    | float | latitude                                                     |
| longitude   | float | longitude                                                    |
| altitude    | float | altitude                                                     |
| statTime    | int   | Last communication time: in milliseconds                     |
| gpsTimeSync | bool  | GPS synchronization status: true (synchronized), false (not synchronized) |

### 6.2 Online heartbeat push notifications
...
It contains fields for online status:
...
System software version (UDP direct connection mode with no data):
...
| hardwareVersion | string | hardware version   |
| firmwareVersion | string | firmware version   |
| firmwareDate    | string | firmware date      |
| kernelVersion   | string | kernel version     |
| model           | string | Gateway Mode       |

System monitoring related fields (no data in UDP direct connection mode):
...
| pfwRunning    | bool    | Gateway pkf usage status: true (online), false (offline) |
| cpuUsage      | []float | CPU load                                                 |
| memUsage      | float   | Memory usage rate (percentage)                           |
| rootDiskUsage | float   | Root partition usage rate (percentage)                   |
| logDiskUsage  | float   | Log partition usage rate (percentage)                    |
| sdkUptime     | int     | Bridge operation duration in seconds                     |
| uptime        | int     | Gateway running duration in seconds                      |

Network-related fields (no data in UDP direct connection mode):
...
| wanIp            | string | Public IP                                 |
| localIp          | string | Intranet IP                               |
| networkDelay     | float  | Ping delay time, measured in milliseconds |
| ethState         | bool   | Is it using Ethernet                      |
| lteState         | bool   | Is it using 4G                            |