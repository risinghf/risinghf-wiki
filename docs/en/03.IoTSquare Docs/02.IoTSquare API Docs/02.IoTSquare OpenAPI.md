...
| productType   | string | device product type：rhf3mr01(relay),rhf3m485(High Isolation RS485 Data Forward Unit),lorawan；3.4.15 Added |
| deviceProfileId | uint | device profile ID, optional, standard relay configuration; 4.3.1 Added |
...
| data            | string    | uplink package data encoded with BASE64                                                        |
...
|------------|--------|----------|------------------------------------------------------------------------------------------|
| devEUI     | string | No       | 16-byte hexadecimal string, device unique identifier. If not provided, returns the downlink queue size of all devices |
...
| gwName       | string            | gateway name                                                                                 |
| gwType       | string            | gateway type                                                                                 |
| region       | string            | frequency region, e.g. `cn470`; added in 4.3.2                                                  |
| subnet       | string            | subnet, e.g. `ch_08-15`; added in 4.3.2                                                        |
| enableClassB | bool              | whether to enable Class B                                                                    |
| enablePFW    | bool              | whether to enable pktfwd  ;3.4 deprecated                                                    |
...
### 5.2.7 Gateway Network Status(4.2.3 Added)

**URL：**`https://cloud.iotsquare.xyz/openapi/gateway/status/network`
...
| Field Name | Type   | Required | Description                                                      |
|------------|--------|----------|------------------------------------------------------------------|
| gwID       | string | Yes      | gateway ID                                                       |

**Response Parameter**

| Field Name  | Type    | Description                                                                                                    |
|-------------|---------|----------------------------------------------------------------------------------------------------------------|
| nwkGw       | string  | gateway interface name                                                                                         |
| localIP     | string  | local IP (pico reuses lteNetIP)                                                                                |
| wanIP       | string  | client-side public IP                                                                                          |
| nwkDelay    | string  | network delay (ping result, serialized as string format, e.g. "15ms")                                          |
| ltess       | string  | 4G signal strength, obtained via get_lterssi command (pico reuses lteNetRssi)                                  |
| sensorData  | string  | sensor data (original struct field name spelling corrected to standard naming)                                  |
| createtime  | string  | data report time (serialized as RFC3339 format, e.g. "2024-05-20T13:14:15+08:00")                             |
| ethState    | boolean | ethernet connection status: true=connected, false=disconnected                                                 |
| lteState    | boolean | LTE dial status: true=dial successful, false=not dialed                                                        |
| apn         | string  | APN access point (CMNET/CTNET for 4G connection, empty string for ethernet)                                    |
| mode        | string  | LTE dial network mode (e.g. TDD-LTE/FDD-LTE)                                                                  |
| rssi        | number  | LTE current network signal quality (unit: dBm, valid range: -113~-51)                                          |
| rsrp        | number  | LTE reference signal received power (unit: dBm, valid range: -140~-44)                                         |
| rsrq        | number  | LTE reference signal received quality (unit: dB, valid range: -19.5~-3)                                        |
| snr         | number  | LTE signal-to-noise ratio (unit: dB, valid range: -20~30)                                                      |
| imsi        | string  | SIM card IMSI (International Mobile Subscriber Identity, composed of MCC/MNC/MSIN)                             |
| cellid      | string  | LTE dial cell ID                                                                                               |
| lacid       | string  | LTE dial location area ID                                                                                      |
| imei        | string  | LTE module IMEI (device identification code)                                                                   |
| iccid       | string  | SIM card ICCID (Integrated Circuit Card Identification)                                                        |
| dialStatus  | integer | SIM card and dial status: 0=no SIM card, 1=SIM card not recognized, 2=not dialed, 3=dial successful           |
| lteManuf    | string  | LTE module manufacturer (e.g. Huawei, Quectel)                                                                 |
| lteModel    | string  | LTE module model (e.g. EC20, AG35)                                                                             |
| lteRevision | string  | LTE module firmware version                                                                                    |
| lteEsn      | string  | LTE module ESN (Electronic Serial Number)                                                                      |
| lteGcap     | string  | LTE module capability set (e.g. supported frequency bands)                                                     |
| meid        | string  | LTE module MEID (Mobile Equipment Identifier)                                                                  |
| rxlev       | string  | LTE dial receive level (unit: dBm)                                                                             |
| lteSim      | string  | SIM type used for dialing. esim=built-in SIM card, usim=external SIM card (4.3.2 Added)                        |
...
```json
{
  ...
  "data": {
    "nwkType": "eth0",
    "wanIP": "113.89.107.104",
    "lanIP": "172.18.2.9",
    "ltess": "-115",
    "nwkDelay": "8.384439ms",
    "sensorData": "",
    "createtime": 1764847400770,
    "ethState": true,
    "lteState": false,
    "apn": "",
    "mode": "",
...
### 5.2.8 Gateway System Status(4.2.3 Added)

**URL：**`https://cloud.iotsquare.xyz/openapi/gateway/status/system`
...
| Field Name              | Type                | Description                                                                  |
|-------------------------|---------------------|------------------------------------------------------------------------------|
| uptime                  | int                 | gateway uptime after boot, in seconds                                        |
| sdkUptime               | int                 | gateway bridge uptime after boot                                             |
| cpuUsage                | string              | CPU usage                                                                    |
| memUsage                | string              | memory usage                                                                 |
| rootDiskUsage           | string              | root partition usage                                                         |
| logDiskUsage            | string              | log partition usage                                                          |
| pktfwdRunning           | bool                | whether pktfwd is running                                                    |
| createtime              | uint64              | latest information report timestamp in milliseconds                          |
| temperature             | string              | temperature                                                                  |
| powerState              | bool                | true=external power connected, false=external power disconnected             |
| powerVoltage            | float64             | external power voltage, unit: volt (4.3.2 Added)                            |
| batteryState            | bool                | true=backup lithium battery connected, false=not connected                   |
| batteryLevel            | uint                | backup lithium battery level percentage, range: 0~100                        |
| batteryVoltage          | float64             | backup lithium battery voltage, unit: volt                                   |
| sbatteryState           | bool                | true=solar battery connected, false=not connected (4.3.2 Added)             |
| sbatteryLevel           | uint                | solar battery level percentage, range: 0~100                                 |
| sbatteryVoltage         | float64             | solar battery voltage, unit: volt                                            |
| configUpdateFlag        | bool                | configuration update flag (4.3.2 Added)                                      |
| rstSrc                  | int                 | reset source (4.3.2 Added)                                                   |
| rstSrcUpdatetime        | int64               | reset source update time, unit: ms (4.3.2 Added)                            |
| configUpgradeUpdatetime | int64               | configuration upgrade update time, unit: ms (4.3.2 Added)                    |
| sbatteryCollectObject   | SbatteryCollectData | solar battery collection data (4.3.2 Added)                                  |

**SbatteryCollectData structure** (4.3.2 Added):

| Field Name   | Type                  | Description                |
|--------------|-----------------------|----------------------------|
| batteryBasic | SbatteryBasic         | battery basic information  |
| batteryCell  | SbatteryCell          | battery cell information   |
| generalParam | SbatteryGeneralParam  | general parameters         |
| miscParam    | SbatteryMiscParam     | misc parameters            |

**SbatteryBasic structure**:

| Field Name                    | Type                | Description                                  |
|-------------------------------|---------------------|----------------------------------------------|
| controlInfo                   | SbatteryControlInfo | control information                          |
| warnInfo                      | SbatteryWarnInfo    | warning information                          |
| remainingAh                   | float64             | remaining Ah                                 |
| soc                           | float64             | state of charge (SoC)                        |
| sohPwm                        | float64             | PWM duty cycle                               |
| power                         | float64             | battery power                                |
| totalVoltage                  | float64             | battery total voltage                        |
| totalCurrent                  | float64             | battery total current                        |
| totalCurrentDirection         | string              | battery total current direction              |
| pvChargeCurrent               | float64             | PV charge current (solar side)               |
| pvChargeCurrentDirection      | string              | PV charge current direction (solar side)     |
| loadDischargeCurrent          | float64             | load discharge current                       |
| loadDischargeCurrentDirection | string              | load discharge current direction             |
| temp1                         | float64             | temperature 1                                |
| temp2                         | float64             | temperature 2                                |
| temp3                         | float64             | temperature 3                                |
| clampingVoltage               | float64             | clamping voltage                             |
| soh                           | float64             | state of health (SOH)                        |
| socPwm                        | float64             | PWM duty cycle                               |
| pvVoltage                     | float64             | PV voltage                                   |
| pvCurrent                     | float64             | PV current                                   |
| pvCurrentDirection            | string              | PV current direction                         |
| gridCurrent                   | float64             | grid current                                 |
| gridCurrentDirection          | string              | grid current direction                       |
| tempProtect                   | float64             | temperature protection                       |
| dischargeProtect              | float64             | discharge protection                         |
| chargeProtect                 | float64             | charge protection                            |

**SbatteryControlInfo structure**:

| Field Name      | Type   | Description          |
|-----------------|--------|----------------------|
| heating         | string | heating              |
| heatDissipation | string | heat dissipation     |
| buzzing         | string | buzzing              |
| overTempProt    | string | over-temperature protection |
| dischargeSwitch | string | discharge switch     |
| chargingSwitch  | string | charging switch      |
| chargingOn      | string | charging on          |
| chargingConn    | string | charging connection  |

**SbatteryWarnInfo structure**:

| Field Name                 | Type    | Description                    |
|----------------------------|---------|--------------------------------|
| highTempProt               | string  | high temperature protection    |
| severeOvercurrent          | string  | severe overcurrent             |
| chargingOvercurrent        | string  | charging overcurrent           |
| batteryOvercharge          | string  | battery overcharge             |
| batteryDepleted            | string  | battery depleted               |
| highTempWarn               | string  | high temperature warning       |
| dischargeOvercurrent       | string  | discharge overcurrent          |
| lowBattery                 | string  | low battery                    |
| excessivePressure          | string  | excessive pressure             |
| lowTempCharging            | string  | low temperature charging       |
| lowTempDischarge           | string  | low temperature discharge      |
| chargingExcessivePressure  | string  | charging excessive pressure    |
| shortCircuitFault          | string  | short circuit fault            |
| highHumidity               | string  | high humidity                  |
| insulationTestAlarm        | string  | insulation test alarm          |
| incompatibleCharger        | string  | incompatible charger           |
| faultCodes                 | float64 | fault codes                    |
| faultLevel                 | float64 | fault level                    |

**SbatteryCell structure**:

| Field Name   | Type    | Description           |
|--------------|---------|-----------------------|
| cellCount    | float64 | cell count            |
| cell1Voltage | float64 | cell 1 voltage        |
| cell2Voltage | float64 | cell 2 voltage        |
| cell3Voltage | float64 | cell 3 voltage        |

**SbatteryGeneralParam structure**:

| Field Name             | Type               | Description                              |
|------------------------|--------------------|------------------------------------------|
| lvl1DischargeOc        | float64            | level 1 discharge overcurrent            |
| lvl1DischargeOcDelay   | float64            | level 1 discharge overcurrent delay      |
| lvl2DischargeOc        | float64            | level 2 discharge overcurrent            |
| lvl2DischargeOcDelay   | float64            | level 2 discharge overcurrent delay      |
| cellUv                 | float64            | cell under-voltage                       |
| cellUvRecover          | float64            | cell under-voltage recovery              |
| cellOvRecover          | float64            | cell over-voltage recovery               |
| cellOv                 | float64            | cell over-voltage                        |
| batOtAlarm             | float64            | battery over-temperature alarm           |
| batOtProtect           | float64            | battery over-temperature protection      |
| sysOtAlarm             | float64            | system over-temperature alarm            |
| sysOtProtect           | float64            | system over-temperature protection       |
| cellCount              | float64            | battery cell count                       |
| ratedCapacity          | float64            | rated capacity                           |
| currentRange           | float64            | current range                            |
| currentOffset          | float64            | current zero offset                      |
| voltageCalib           | float64            | voltage calibration coefficient          |
| chargeCurrentOffset    | float64            | charge current zero offset               |
| currentNoiseThreshold  | float64            | current noise threshold                  |
| chargeOc               | float64            | charge overcurrent threshold             |
| fullVoltage            | float64            | battery full voltage                     |
| floatVoltage           | float64            | cell float voltage                       |
| balanceStartVoltage    | float64            | balance start voltage                    |
| chargeLtProtect        | float64            | charge low-temperature protection        |
| dischargeLtProtect     | float64            | discharge low-temperature protection     |
| heatStartTemp          | float64            | heating trigger temperature              |
| heatStopTemp           | float64            | heating stop temperature                 |
| chargeLtRecover        | float64            | charge low-temperature recovery          |
| dischargeLtRecover     | float64            | discharge low-temperature recovery       |
| tempSensorEnable       | float64            | temperature sensor enable                |
| funcSwitch             | SbatteryFuncSwitch | function switch                          |
| diffVoltageSection     | float64            | differential voltage alarm section       |
| diffVoltageHigh        | float64            | high section differential voltage alarm  |
| diffVoltageLow         | float64            | low section differential voltage alarm   |
| gridSwitchVoltage      | float64            | grid switch voltage                      |
| chargeDiffHigh         | float64            | charge high section differential alarm   |
| devId                  | string             | device ID                                |
| totalUv                | float64            | total under-voltage protection           |
| totalOv                | float64            | total over-voltage protection            |
| sensorEnable           | float64            | sensor enable                            |
| dischargeOt            | float64            | discharge over-temperature protection    |
| socRecover             | float64            | over-discharge recovery SoC              |
| gridCurrentOffset      | float64            | grid current offset                      |
| chargeDiffLow          | float64            | charge low section differential alarm    |
| gridCurrentRange       | float64            | grid current range                       |
| loadCurrentRange       | float64            | load current range                       |

**SbatteryFuncSwitch structure**:

| Field Name        | Type   | Description          |
|-------------------|--------|----------------------|
| batteryPriority   | string | battery priority     |
| reportRealtimeInfo | string | report realtime info |
| batteryCells      | string | battery cells        |
| shortCircuitProt  | string | short circuit protection |

**SbatteryMiscParam structure**:

| Field Name  | Type    | Description                     |
|-------------|---------|---------------------------------|
| dischargeAh | float64 | cumulative discharge Ah         |
| calibAh     | float64 | calibration Ah                  |
| remainAh    | float64 | remaining Ah                    |
| chargeAh    | float64 | cumulative charge Ah            |
| pvDaily     | float64 | PV daily generation             |
| pvTotal     | float64 | PV total generation             |
| loadDaily   | float64 | load daily consumption          |
| loadTotal   | float64 | load total consumption          |
| forceCtrl   | float64 | charge/discharge force switch   |
...
```json
{
  ...
  "data": {
    "uptime": 337,
    "sdkUptime": 309,
    "cpuUsage": "0.64 0.38 0.18",
    "memUsage": "213.34/924.22MB 23.08%",
    "rootDiskUsage": "3511.63/29065.87MB 12.08%",
    "logDiskUsage": "0.50/100.00MB 0.50%",
    "pktfwdRunning": true,
    "createtime": 1764847404435,
    "temperature": "36.56",
    "powerState": true,
    "powerVoltage": 11.1,
    "batteryState": false,
    ...
    "rstSrc": 0,
    "rstSrcUpdatetime": -1,
    "configUpgradeUpdatetime": -1,
    "sbatteryCollectObject": {
      "batteryBasic": {
        "controlInfo": {
          "heating": "0",
          "heatDissipation": "0",
        ...
        "warnInfo": {
          ...
          "lowTempCharging": "0",
          "lowTempDischarge": "0",
          "chargingExcessivePressure": "0",
          "shortCircuitFault": "0",
          "highHumidity": "0",
          "insulationTestAlarm": "0",
          "incompatibleCharger": "0",
          "faultCodes": 0,
          "faultLevel": 0
        },
        "remainingAh": 0,
        ...
        "pvChargeCurrent": 0,
        "pvChargeCurrentDirection": "",
        "loadDischargeCurrent": 0,
        "loadDischargeCurrentDirection": "",
        "temp1": 0,
        "temp2": 0,
        "temp3": 0,
        "clampingVoltage": 0,
